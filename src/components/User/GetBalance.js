...
const GetBalance = () => {
  ...
  const submitHandler = async (event) => {
    ...
    const response = await axios.get(`http://localhost:3000/api/user/getBalance/${emailValue}`,{withCredentials: true,headers: {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'}});
...
<li><b className="text-red-900">Token Name:</b> {id.Token}</li>
<li><b className="text-red-900">Balance:</b> {id.Balance}</li>
</ul>
            ))}
          </p>
}
