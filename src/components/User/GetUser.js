...
isEmailVerified:''
  ...
  const submitHandler = async (event) => {
    ...
    const response = await axios.get(`http://localhost:3000/api/user/getUser/${emailValue}`,{withCredentials: true,headers: {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'
  }});
  ...
  return (
    <div>
      <div className="flex flex-col items-center min-h-screen pt-6 sm:justify-center sm:pt-0 ">
        ...
        {userDetail.name && <ul className="mt-10 text-blue-900"><li><b className="text-red-900"> Name:</b> {userDetail.name}</li>
        <li><b className="text-red-900">Role:</b> {userDetail.role}</li>
        <li> <b className="text-red-900">Wallet Address: </b>{userDetail.walletAddress}</li>
        <li><b className="text-red-900"> Email:</b> {userDetail.email}</li>
        <li><b className="text-red-900">Email verification status: </b>{userDetail.isEmailVerified}</li>
        </ul>}
      </div>
    </div>
  );
