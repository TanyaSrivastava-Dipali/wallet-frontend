...
const Verify = () => {
  ...
  const submitHandler = async (event) => {
    ...
    let response = await fetch("/api/user/verifyEmail", {
      method: "POST",
  ...
  const resendOtpHandler = async (e) => {
    e.preventDefault();
    let response = await fetch("/api/user/getOtpForEmailConfirmation", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        email: emailValue,
          ...
          <form onSubmit={submitHandler}>
            ...
            <div className="mt-5">
              ...
              <div className="flex flex-col items-start">
                ...
                />
                {otpHasError && (
                  <p className="text-red-700">OTP can't be empty.</p>
                )}
              </div>
            </div>
            <div className="flex justify-center items-center mt-5">
