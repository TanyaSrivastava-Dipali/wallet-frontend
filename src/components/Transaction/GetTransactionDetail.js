...
const Transaction = () => {
  ...
  const submitHandler = async (event) => {
    ...
    const response = await axios.get(
      `http://localhost:3000/api/transaction/getTransactionDetail/${hashValue}`,
      {
      ...
      <div className="flex flex-col items-center min-h-screen pt-6 sm:justify-center  sm:pt-0">
        ...
        {transactionDetail.amount && (
          <ul className="mt-10 text-blue-900">
            <li>
              <b className="text-red-900"> Sender:</b>{" "}
              {transactionDetail.sender}
            </li>
            <li>
              <b className="text-red-900">Receiver:</b>{" "}
              {transactionDetail.receiver}
            </li>
            <li>
              {" "}
              <b className="text-red-900">SenderWalletAddress: </b>
              {transactionDetail.SenderWalletAddress}
            </li>
            <li>
              <b className="text-red-900"> ReceiverWalletAddress:</b>{" "}
              {transactionDetail.ReceiverWalletAddress}
            </li>
            <li>
              <b className="text-red-900">Transaction Hash: </b>
              {transactionDetail.ethTRXHash}
            </li>
            <li>
              <b className="text-red-900">Token name : </b>
              {transactionDetail.token}
            </li>
            <li>
              <b className="text-red-900">Transaction trxTimeStamp </b>
              {transactionDetail.trxTimeStamp}
            </li>
          </ul>
        )}
      </div>
