  ...
  useEffect(() => {
    const getTrx = async () => {
      const response = await axios.get(
        "http://localhost:3000/api/transaction/getAllTransactions",
        {
        ...
        <p>
          {trx.map((t) => (
            <ul className="mt-10 text-black border-solid border-2 border-blue-900 ">
              <li>
                <b className="text-red-900"> key :</b>
                {t.ethTRXHash}
              </li>
              <li>
              ...
              <li>
                <b className="text-red-900"> Sender Wallet Address:</b>
                {t.SenderWalletAddress}
              </li>
              <li>
                <b className="text-red-900"> Receiver Wallet Address:</b>
                {t.ReceiverWalletAddress}
              </li>
              <li>
                <b className="text-red-900"> amount:</b>
                {t.amount}
              </li>
              <li>
                <b className="text-red-900"> Token:</b>
                {t.token}
              </li>
              <li>
                <b className="text-red-900"> Eth Transaction hash:</b>
                {t.ethTRXHash}
              </li>
            </ul>
          ))}
